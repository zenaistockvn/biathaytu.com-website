...
import Festbier5LProductView from '../../components/Festbier5LProductView';
import { getTastingNotes } from '../../utils/getTastingNotes';
import { toAbsoluteSiteUrl } from '@/lib/seo/site';

const FESTBIER_5L_SLUG = 'benediktiner-festbier-bom-5l';
const FESTBIER_5L_DESCRIPTION = 'Benediktiner Festbier Bom 5L, bia lễ hội Đức 5.8% ABV với nền malt đầy đặn, cảm giác mượt và hậu vị cân bằng. Xem cách làm lạnh, cách rót và thông tin tư vấn tại Bia Thầy Tu.';
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const isFestbier5L = product.slug === FESTBIER_5L_SLUG;

  const ogImageUrl = toAbsoluteSiteUrl(
    isFestbier5L
      ? '/images/brand/benediktiner-official/festbier-keg-nobg.webp'
      : product.images?.[0] || '/images/sanh_bia_duc_cover.png',
  );
  const pageDescription = isFestbier5L
    ? FESTBIER_5L_DESCRIPTION
    : product.description || `Khám phá hương vị và thông tin chi tiết của ${product.name}. Liên hệ Bia Thầy Tu để được tư vấn sản phẩm.`;
  const pageTitle = isFestbier5L
    ? 'Benediktiner Festbier Bom 5L – Bia Lễ Hội Đức 5.8%'
    : product.name;
...
export default async function ProductDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const productUrl = `https://www.biathaytu.com/san-pham/${product.slug || product.id}`;
  const schemaDescription = isFestbier5L ? FESTBIER_5L_DESCRIPTION : product.description || undefined;
  const productSchema = {
    ...getProductSchema({
      id: product.id,
      name: product.name,
  ...
  return (
    <div className={isFestbier5L ? undefined : 'subpage-wrap'}>
