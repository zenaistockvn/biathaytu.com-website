    ...
    <div className="product-accordion" style={{ marginTop: '40px', borderTop: '1px solid var(--web-border)' }}>
        ...
        {openSection === 'storage' && (
          <div style={{ paddingBottom: '20px', color: 'var(--web-text-muted)', lineHeight: '1.6' }}>
            <ul style={{ paddingLeft: '20px', margin: 0 }}>
              <li style={{ marginBottom: '8px' }}>Bảo quản ở nơi khô ráo, thoáng mát, tránh tiếp xúc trực tiếp với ánh nắng mặt trời.</li>
              <li style={{ marginBottom: '8px' }}>Nên giữ lạnh ở nhiệt độ 5 - 8 độ C (ngăn mát tủ lạnh) trước khi uống tối thiểu 24 giờ để bia đạt độ ngon tuyệt đối.</li>
              <li style={{ marginBottom: '8px' }}>Tuyệt đối tránh để bia bị sốc nhiệt (chuyển đột ngột từ nóng sang lạnh hoặc ngược lại) hay đóng băng trong ngăn đá.</li>
            </ul>
          </div>
        )}
      </div>
      ...
      <div className="accordion-item" style={{ borderBottom: '1px solid var(--web-border)' }}>
        <button 
          ...
          style={{ width: '100%', padding: '20px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'none', border: 'none', cursor: 'pointer', fontSize: '18px', fontWeight: 600, color: 'var(--web-text)' }}
        >
          <span>🍻 Cách thưởng thức ngon nhất</span>
          <span>{openSection === 'drink' ? '−' : '+'}</span>
        </button>
        {openSection === 'drink' && (
          <div style={{ paddingBottom: '20px', color: 'var(--web-text-muted)', lineHeight: '1.6' }}>
            <ul style={{ paddingLeft: '20px', margin: 0 }}>
              <li style={{ marginBottom: '8px' }}><strong>Nhiệt độ lý tưởng:</strong> Ướp lạnh bia từ 5 - 8 độ C.</li>
              <li style={{ marginBottom: '8px' }}><strong>Tuyệt đối không dùng đá:</strong> Tránh uống cùng với đá lạnh để giữ nguyên độ tinh khiết và cấu trúc bọt nguyên bản của bia Đức.</li>
              <li style={{ marginBottom: '8px' }}><strong>Sử dụng ly phù hợp:</strong> Nên dùng ly thủy tinh đã được làm lạnh. Ly phải sạch hoàn toàn, không có vết dầu mỡ để bọt bia có thể giữ được lâu.</li>
              <li style={{ marginBottom: '8px' }}><strong>Nghệ thuật rót (Với dòng Weissbier/Lúa mì):</strong> Rót nghiêng ly 45 độ. Khi còn khoảng 2-3cm bia cuối chai, hãy lắc nhẹ chai theo vòng tròn để lớp men (Naturtrüb) chìm dưới đáy hòa quyện đều rồi mới rót nốt phần còn lại lên đỉnh ly để tạo lớp bọt vương miện tuyệt đẹp.</li>
            </ul>
          </div>
        )}
      </div>
