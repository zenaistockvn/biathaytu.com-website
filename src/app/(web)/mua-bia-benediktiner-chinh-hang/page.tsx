    ...
    <div className="web-app" style={{ backgroundColor: 'var(--web-bg)' }}>
      ...
      <section style={{ padding: '120px 0 60px', background: 'var(--web-ink)', color: '#fff', textAlign: 'center' }}>
        <div className="container" style={{ maxWidth: '800px' }}>
          <p style={{ fontSize: '13px', fontWeight: 700, letterSpacing: '3px', color: 'var(--web-accent)', textTransform: 'uppercase', marginBottom: '20px' }}>Đặt Hàng Chính Hãng</p>
      ...
      <article className="container" style={{ maxWidth: '850px', padding: '60px 20px 40px' }}>
        ...
        <p style={{ marginTop: '12px', marginBottom: '24px' }}>Chưa rõ Benediktiner chính hãng là gì? Đọc <a href="/bia-benediktiner-chinh-hang" style={{ color: 'var(--web-accent-strong)', fontWeight: 600 }}>cách nhận biết bia Benediktiner chính hãng →</a></p>

        <h2 style={{ fontSize: '28px', fontWeight: 700, color: 'var(--web-ink)', marginBottom: '24px', fontFamily: 'var(--font-serif)' }}>Bảng Giá Bia Benediktiner 2026</h2>
        <div style={{ overflowX: 'auto', marginBottom: '40px' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '15px' }}>
            <thead>
              <tr style={{ background: 'var(--web-ink)', color: '#fff' }}>
            ...
            <tbody>
              {prices.map((item, i) => (
                <tr key={i} style={{ borderBottom: '1px solid var(--web-border)', background: i % 2 === 0 ? '#fff' : 'var(--web-bg-section)' }}>
                  <td style={{ padding: '14px 16px', fontWeight: 600, color: 'var(--web-ink)' }}>{item.name}</td>
                  <td style={{ padding: '14px 16px', color: 'var(--web-text-muted)' }}>{item.specs}</td>
                  <td style={{ padding: '14px 16px', textAlign: 'right', fontWeight: 700, color: 'var(--web-accent-strong)' }}>{item.price}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <h2 style={{ fontSize: '24px', fontWeight: 700, color: 'var(--web-ink)', marginBottom: '16px', fontFamily: 'var(--font-serif)' }}>Chính sách giao hàng</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px', marginBottom: '40px' }}>
          {[
            ...
            <div key={i} style={{ padding: '20px', background: '#fff', borderRadius: '12px', border: '1px solid var(--web-border)' }}>
              ...
              <h3 style={{ fontSize: '15px', fontWeight: 700, color: 'var(--web-ink)', marginBottom: '6px' }}>{item.title}</h3>
        ...
        <div style={{ padding: '32px', background: 'var(--web-ink)', borderRadius: '16px', color: '#fff', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="https://zalo.me/0899191313" target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', padding: '14px 32px', background: 'var(--web-accent)', color: 'var(--web-ink)', fontWeight: 700, borderRadius: '8px', textDecoration: 'none' }}>Nhắn Zalo 0899.191.313</a>
            <a href="tel:0899191313" style={{ display: 'inline-block', padding: '14px 32px', border: '2px solid var(--web-accent)', color: 'var(--web-accent)', fontWeight: 700, borderRadius: '8px', textDecoration: 'none' }}>Gọi Hotline</a>
          </div>
        </div>
      </article>
