...
function scan() {
  ...
  for (const file of walk('src', /\.tsx$/)) {
    ...
    for (const block of src.match(/<style[^>]*>[\s\S]*?<\/style>|__html:\s*`[\s\S]*?`/g) || []) {
    ...
    src.split('\n').forEach((line, i) => {
      for (const p of line.matchAll(TSX_COLOR_PROP)) {
        for (const m of p[3].match(COLOR) || []) if (!THIRD_PARTY.test(m)) findings.push({ file, line: i + 1, value: m, kind: 'tsx-inline' });
      }
    });
  }
  return findings;
